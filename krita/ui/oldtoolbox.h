 ...
 */
#ifndef _OLD_TOOLBOX_H_
#define _OLD_TOOLBOX_H_
...
/**
 * OldToolBox is a kind of super-specialized toolbox that can order
 * tools according to type and priority.
 *
 */
class OldToolBox : public KToolBar {

    Q_OBJECT

public:

    OldToolBox( KMainWindow *mainWin, const char* name, KInstance* instance, int numberOfTooltypes);
    virtual ~OldToolBox();
...
private:
    ...
    QList<OldToolArea *> m_toolBoxes; // For every OldToolArea
...
class OldToolArea : public QWidget {

public:
    OldToolArea(QWidget *parent);
    ~OldToolArea();
