...
void KoSvgTextShape::relayout() const
{
        ...
        for (KoSvgTextChunkShapeLayoutInterface::SubChunk chunk : textChunks) {
            ...
            for (int i = 0; i < length; i++) {
                ...
                if (lineBreaks[start + i] == LINEBREAK_MUSTBREAK) {
                    ...
                    cr.collapseIfAtEndOfLine =
                        KoCssTextUtils::collapseLastSpace(text.at(start + i),
                                                          collapse);
                }
    ...
    for (int i = 0; i < result.size(); i++) {
        if (result.at(i).typographic_index != -1) {
            ...
            result[firstCluster].collapseIfAtEndOfLine =
                result.at(i).collapseIfAtEndOfLine;
 ...
 */
int addWordToLine(QVector<CharacterResult> &result,
                  QPointF &currentPos,
                                         ...
                                         QVector<CharacterResult> &result)
{
    ...
    while (ltr ? it.hasNext() : it.hasPrevious()) {
        ...
        qreal pos = isHorizontal ? charResult.cssPosition.x()
                                 : charResult.cssPosition.y();
        qreal advance =
            isHorizontal ? charResult.advance.x() : charResult.advance.y();
        ...
        QPointF wordAdvance = ltr
            ? isHorizontal ? QPoint(b - a, 0) : QPoint(0, b - a)
            : isHorizontal ? QPoint(a - b, 0)
                           : QPoint(0, a - b);
