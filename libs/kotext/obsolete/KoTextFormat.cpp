...
void KoTextFormat::load( KoOasisContext& context )
{
    ...
    if ( styleStack.hasProperty( KoXmlNS::fo, "color" ) ) { // 3.10.3
        col.setNamedColor( styleStack.property( KoXmlNS::fo, "color" ) ); // #rrggbb format
    }
    ...
    if( styleStack.hasProperty( KoXmlNS::style, "text-position")) { // OO 3.10.7
        importTextPosition( styleStack.property( KoXmlNS::style, "text-position"), fn.pointSizeF(),
    ...
    if ( styleStack.hasProperty( KoXmlNS::fo, "background-color") ) {
        QString tmp = styleStack.property( KoXmlNS::fo, "background-color");
        if (tmp != "transparent")
