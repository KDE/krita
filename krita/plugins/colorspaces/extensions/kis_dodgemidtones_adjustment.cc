...
public:
    ...
    void transform(const quint8 *srcU8, quint8 *dstU8, qint32 nPixels) const
    {
        ...
        while(nPixels > 0)
        {   
            r = pow(KoColorSpaceMaths<_channel_type_, float>::scaleToA(src->red), exposure);
            g = pow(KoColorSpaceMaths<_channel_type_, float>::scaleToA(src->green), exposure);
            b = pow(KoColorSpaceMaths<_channel_type_, float>::scaleToA(src->blue), exposure);
