...
//assignment of doubles to ElType is defined
    ...
    void ludcmp(matrix<ElType>& a, vector<int>& indx,double& d)
    {
        ...
        {
            big=0.0;
//         kDebug(41001) << "new search" << endl;
            for (j=0;j<n;j++) { if ((temp=fabs(a[i][j])) > big) big=temp;
                /*            kDebug(41001) << temp << " " << fabs(a[i][j]) << " "<< big <<endl; */}
...
double* KisPerspectiveMath::computeMatrixTransfoToPerspective(const QPointF& topLeft, const QPointF& topRight, const QPointF& bottomLeft, const QPointF& bottomRight, const QRect& r)
{
...
double* KisPerspectiveMath::computeMatrixTransfoFromPerspective(const QRect& r, const QPointF& topLeft, const QPointF& topRight, const QPointF& bottomLeft, const QPointF& bottomRight)
{
