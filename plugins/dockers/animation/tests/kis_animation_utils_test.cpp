...
void KisAnimationUtilsTest::test()
{
    ...
    QScopedPointer<KUndo2Command> cmd(createMoveKeyframesCommand(frameMoves, MoveKeyframes, false));
    cmd->redo();
    ...
    cmd.reset(createMoveKeyframesCommand(frameMoves, MoveKeyframes, false));
    cmd->redo();
