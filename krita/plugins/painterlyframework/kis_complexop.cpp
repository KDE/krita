...
void KisComplexOp::paintAt(const KisPaintInformation& info)
{
    ...
    adjustedInfo.setPressure(pressure);
    ...
    KisPaintDeviceSP buffer = new KisPaintDevice(cs->clone());
    buffer->createPainterlyOverlay();

    QPointF hotSpot = brush->hotSpot(adjustedInfo.pos().x(),adjustedInfo.pos().y());
    ...
    if (brush->brushType() == IMAGE || brush->brushType() == PIPE_IMAGE) {
        dab = brush->image(cs, 1, 1, adjustedInfo, xFraction, yFraction);
    } else {
        dab = new KisPaintDevice(cs->clone());
        brush->mask(dab, 1, 1, 0, adjustedInfo, xFraction, yFraction);
    }
    {
        KisPainter p(buffer);
    ...
    QSize sz(brush->width(), brush->height());
...
//                     brushCell->volume = 1000.0;
    ...
    painter()->bltSelection(dstRect.x(), dstRect.y(), painter()->compositeOp(), buffer, painter()->opacity(), sx, sy, sw, sh);
...
{
    const KoColorSpace *overlaycs = KisPainterlyOverlayColorSpace::instance();
