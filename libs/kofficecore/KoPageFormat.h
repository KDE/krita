...
namespace KoPageFormat
{
     ...
     */
    KOFFICECORE_EXPORT KPrinter::PageSize printerPageSize( Format format );

    /**
     * Returns the width (in mm) for a given page format and orientation
     * 'Custom' isn't supported by this function, obviously.
     */
    KOFFICECORE_EXPORT double width( Format format, Orientation orientation = Landscape);

    /**
     * Returns the height (in mm) for a given page format and orientation
     * 'Custom' isn't supported by this function, obviously.
     */
    KOFFICECORE_EXPORT double height( Format format, Orientation orientation  = Landscape);

    /**
     * Returns the internal name of the given page format.
     * Use for saving.
     */
    KOFFICECORE_EXPORT QString formatString( Format format );

    /**
     * Convert a format string (internal name) to a page format value.
     * Use for loading.
     */
    KOFFICECORE_EXPORT Format formatFromString( const QString & string );

    /**
     * Returns the default format (based on the KControl settings)
     */
    KOFFICECORE_EXPORT Format defaultFormat();

    /**
     * Returns the translated name of the given page format.
     * Use for showing the user.
     */
    KOFFICECORE_EXPORT QString name( Format format );
     ...
     */
    KOFFICECORE_EXPORT Format guessFormat( double width, double height );
}
