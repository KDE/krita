    ...
    , m_textData(shape->getMemento())
{
    ...
    if (type == OffsetAll) {
        ...
        const QLineF originalLine(rtl? last.finalPos: first.finalPos, rtl? first.finalPos: last.finalPos);
        ...
        QTransform deltaTf = origin.inverted();
        ...
        while (!originalTf.isEmpty()) {
            ...
            if (type != ScaleOnly) {
                rotations.append(tf.rotateDeg + (angle));
            } else {
                rotations.append(tf.rotateDeg);
            }
        }
