...
{
    if (brushType() == MASK) {
        return KisBrush::paintDevice(colorSpace, scale, ratio, angle, info, subPixelX, subPixelY);
    }
    else { /* if (brushType() == PIPE_MASK)*/
        return m_brushesPipe->paintDevice(colorSpace, scale, ratio, angle, info, subPixelX, subPixelY);
    }
}
