...
class KisWaveCurve : public KisCurveFunction {
    public:
        KisWaveCurve(int amplitude, int wavelenght, int shift, int y, int ystart, int height) : m_amplitude(amplitude), m_wavelength(wavelenght), m_shift(shift), m_y(y), m_ystart(ystart), m_yend(m_ystart + height - 1)
        {
        }
        virtual KisPoint valueAt(int i)
        {
            double yv = m_y + m_amplitude * cos( m_shift + (double)i / m_wavelength );
            if(yv < m_ystart) yv = m_ystart;
            else if(yv > m_yend) yv = m_yend;
//             kdDebug() << i << " " << yv << " " << cos( m_shift + (double)i / m_wavelength ) << " " << m_amplitude * cos( m_shift + (double)i / m_wavelength ) << " " << i << endl;
            return KisPoint(i, yv );
        }
        inline void setY(int y) { m_y = y; }
    private:
        int m_amplitude, m_wavelength, m_shift, m_y, m_ystart, m_yend;
};
...
void KisFilterWave::process(KisPaintDeviceSP src, KisPaintDeviceSP dst, KisFilterConfiguration* config, const QRect& rect)
{
    ...
    KisWaveCurve* curve = new KisWaveCurve(amplitude, wavelength, shift, rect2.y(), rect2.y(), rect2.height());
