...
INIT_TEMPLATE = """from .{library_name} import *"""
...
#            action.triggered.connect(self.action_triggered) 
    ...
    def create(self):
        ...
        def full_dir(path): #convenience function
        ...
        values = {SCRIPT_NAME : self.ui.e_name_of_script.text(), 
                    SCRIPT_COMMENT :  comment, 
                    KRITA_ID : "pykrita_"+self.package_name, 
                    SCRIPT_TYPE :SCRIPT_DOCKER if self.ui.rb_docker.isChecked() else SCRIPT_EXTENSION, 
                    MENU_ENTRY:menu_entry, 
        ...
        try:
            ...
            pass # if package directory exists write into it, overwriting existing files. 
        ...
        message.append("Project files were created in the directory<p>%s"%self.target_directory)
        message.append("<h3>Files Created</h3>The following files were created:<p>")
        for f in self.files:
        ...
        message.append("Make a note of these locations before you click ok.<p>")
        message = "\n".join(message)
    ...
    def name_change(self,  text):
        ...
        * update the e_files_display edit
        """
#        text = self.e_name_of_script.text()
        text = text.strip()
        if len(text) == 0 :
            return 
    ...
    def calculate_file_names(self, text):
        ...
        self.package_file = os.path.join(self.package_name,  self.package_name+".py")
