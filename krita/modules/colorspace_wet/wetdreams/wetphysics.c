...
*/
void
wet_pix_combine (WetPixDbl *dst, WetPixDbl *src1, WetPixDbl *src2)
{
...
void
wet_pix_dilute (WetPixDbl *dst, WetPix *src, double dilution)
{
...
void
wet_pix_reduce (WetPixDbl *dst, WetPix *src, double dilution)
{
...
wet_flow (WetLayer *layer)
{
  ...
  for (y = 1; y < height - 1; y++)
      ...
      for (x = 1; x < width - 1; x++)
	{
	  ...
	  ft = (wet_line[x - rs].h + wet_line[x - rs].w) - my_height;
	  fb = (wet_line[x + rs].h + wet_line[x + rs].w) - my_height;
	  fl = (wet_line[x - 1].h + wet_line[x - 1].w) - my_height;
	  fr = (wet_line[x + 1].h + wet_line[x + 1].w) - my_height;

	  fluid[ix] = 0.4 * sqrt (wet_line[x].w * 1.0 / 255.0);

	  /* smooth out the flow a bit */
	  flow_t[ix] = 0.1 * (10 + ft * 0.75 - fb * 0.25);
	  if (flow_t[ix] > 1)
	    flow_t[ix] = 1;
	  if (flow_t[ix] < 0)
	    flow_t[ix] = 0;
	  flow_b[ix] = 0.1 * (10 + fb * 0.75 - ft * 0.25);
	  if (flow_b[ix] > 1)
	    flow_b[ix] = 1;
	  if (flow_b[ix] < 0)
	    flow_b[ix] = 0;
	  flow_l[ix] = 0.1 * (10 + fl * 0.75 - fr * 0.25);
	  if (flow_l[ix] > 1)
	    flow_l[ix] = 1;
	  if (flow_l[ix] < 0)
	    flow_l[ix] = 0;
	  flow_r[ix] = 0.1 * (10 + fr * 0.75 - fl * 0.25);
	  if (flow_r[ix] > 1)
	    flow_r[ix] = 1;
	  if (flow_r[ix] < 0)
	    flow_r[ix] = 0;
  ...
  for (y = 1; y < height - 1; y++)
      ...
      for (x = 1; x < width - 1; x++)
	{
	  if (wet_line[x].w > 0)
	    {
	      wet_pix_reduce (&wet_mix, &wet_old[ix], 1 - outflow[ix]);

	      wet_pix_reduce (&wet_tmp, &wet_old[ix - rs], flow_t[ix]);
	      wet_pix_combine (&wet_mix, &wet_mix, &wet_tmp);
	      wet_pix_reduce (&wet_tmp, &wet_old[ix + rs], flow_b[ix]);
	      wet_pix_combine (&wet_mix, &wet_mix, &wet_tmp);
	      wet_pix_reduce (&wet_tmp, &wet_old[ix - 1], flow_l[ix]);
	      wet_pix_combine (&wet_mix, &wet_mix, &wet_tmp);
	      wet_pix_reduce (&wet_tmp, &wet_old[ix + 1], flow_r[ix]);
	      wet_pix_combine (&wet_mix, &wet_mix, &wet_tmp);

	      wet_pix_from_double (&wet_line[x], &wet_mix);
...
void
wet_adsorb (WetLayer *layer, WetLayer *adsorb)
{
  int x, y;
  WetPix *wet_line = layer->buf;
