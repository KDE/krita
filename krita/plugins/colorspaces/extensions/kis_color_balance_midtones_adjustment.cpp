...
QList< QPair< KoID, KoID > > KisColorBalanceMidtonesAdjustmentFactory::supportedModels() const
{
...
KoColorTransformation* KisColorBalanceMidtonesAdjustmentFactory::createTransformation(const KoColorSpace* colorSpace, QHash<QString, QVariant> parameters) const
{
    KoColorTransformation * adj;
    if (colorSpace->colorModelId() != RGBAColorModelID) {
        kError() << "Unsupported color space " << colorSpace->id() << " in KisColorBalanceMidtonesAdjustment::createTransformation";
        return 0;
    }
