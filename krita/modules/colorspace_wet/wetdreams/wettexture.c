		       ...
		       double blurh, double blurv)
{
  ...
  for (y = 0; y < lheight; y++)
    {
      ...
      for (x = 1; x < width; x++)
	{
	  wet_line[x].h += ((lh - wet_line[x].h) * ibh + 128) >> 8;
	  lh = wet_line[x].h;
	}
      wet_line += rowstride;
    }
  ...
  for (x = 0; x < width; x++)
	...
	{
	  wet_line += rowstride;
	  wet_line[0].h += ((lh - wet_line[0].h) * ibv + 128) >> 8;
	  lh = wet_line[0].h;
	}
    }
#endif
...
void
wet_layer_clone_texture (WetLayer *dst, WetLayer *src)
{
  int x, y;
  int width = src->width;
		      ...
		      double blurh, double blurv)
{
  ...
  wet_layer_maketexture (pack->layers[0],
			 height,
			 blurh, blurv);
