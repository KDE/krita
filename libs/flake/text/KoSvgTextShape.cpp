                   ...
                   QPointF textIndent)
{
    ...
    // We'll need to shift the text-indent to after the first hanging-glyph if
    // there's one at the start of the line.
                           ...
                           bool atEnd)
{
    ...
    if (!lineIndices.isEmpty()) {
        int lastIndex = lineIndices.last();
        ...
        } else if (result.at(lastIndex).lineEnd == HangBehaviour && inlineSize
                   && atEnd) {
            QPointF pos = isHorizontal ? QPointF(endPos.x(), lineOffset.y())
                                       : QPointF(lineOffset.x(), endPos.y());
            if (!ltr) {
                                         ...
                                         QPointF startPos)
{
    ...
    while (ltr ? it.hasNext() : it.hasPrevious()) {
        ...
        if ((charResult.lineEnd == ConditionallyHang
            ...
            && !lineBox.isEmpty()) {
            wordAdvance = ltr ? wordAdvance - charResult.advance
                ...
                if (overflowWrap == KoSvgText::OverflowWrapBreakWord) {
                    ...
                    if (!inlineSize.isAuto
                        ...
                        for (int i : wordIndices) {
                            wordAdvance =
                                result.at(i).cssPosition - partialWordFirstPos;
                            wordAdvance = ltr
                            ...
                            if (wordLength <= inlineSize.customValue) {
                                ...
                                partialWordFirstPos = ltr
                                    ? result.at(i).cssPosition
                                    : result.at(i).cssPosition
                                        + result.at(i).advance;
                                partialWord.append(i);
                            }
    ...
    if (dominantBaseline == KoSvgText::BaselineResetSize
        ...
        if (hb_version_atleast(4, 0, 0)) {
            ...
            hb_ot_metrics_get_position_with_fallback(
                ...
                &baseline2);
            superScript = QPointF(baseline * freetypePixelsToPt,
            ...
            hb_ot_metrics_get_position_with_fallback(
                ...
                &baseline2);
            subScript = QPointF(baseline * freetypePixelsToPt,
                                baseline2 * -freetypePixelsToPt);
    ...
    bool isHorizontal)
{
    ...
    if (decor != KoSvgText::DecorationNone) {
        ...
        if (isHorizontal) {
            underlineOffset =
                QPointF(0,
                        chunkShape->layoutInterface()->getTextDecorationOffset(
                            KoSvgText::DecorationUnderline));
            lineThroughOffset =
                QPointF(0,
                        chunkShape->layoutInterface()->getTextDecorationOffset(
            ...
            underlineOffset =
                QPointF(chunkShape->layoutInterface()->getTextDecorationOffset(
                            KoSvgText::DecorationUnderline),
                        0);
            lineThroughOffset =
                QPointF(chunkShape->layoutInterface()->getTextDecorationOffset(
        ...
        for (int k = i; k < j; k++) {
            ...
            if (isHorizontal) {
                if (charResult.path.isEmpty()) {
                    ...
                    bottom =
                        qMax(bottom, charResult.path.boundingRect().bottom());
                }
            } else {
                if (charResult.path.isEmpty()) {
                    ...
                    bottom =
                        qMin(bottom, charResult.path.boundingRect().left());
                }
            }
            if (firstPos.isEmpty()) {
            ...
            if (charResult.path.isEmpty()) {
                currentRect |=
                    charResult.boundingBox.translated(charResult.finalPosition);
            } else {
                currentRect |= charResult.path.boundingRect().translated(
                    charResult.finalPosition);
            }
        }
        decorationRects.append(currentRect);
        if (isHorizontal) {
        ...
        for (int i = 0; i < decorationRects.size(); i++) {
            ...
            if (decor.testFlag(KoSvgText::DecorationUnderline)) {
                if (currentTextPath) {
                    ...
                    underline.addPath(stretchGlyphOnPath(
                        p.translated(underlineOffset),
                        path,
                        isHorizontal,
                        currentTextPathOffset,
                        currentTextPath->isClosedSubpath(0)));
            ...
            if (decor.testFlag(KoSvgText::DecorationOverline)) {
                if (currentTextPath) {
                    ...
                    overline.addPath(stretchGlyphOnPath(
                        p.translated(overlineOffset - pathWidth),
                        path,
                        isHorizontal,
                        currentTextPathOffset,
                        currentTextPath->isClosedSubpath(0)));
            ...
            if (decor.testFlag(KoSvgText::DecorationLineThrough)) {
                if (currentTextPath) {
                    ...
                    linethrough.addPath(stretchGlyphOnPath(
                        p.translated(lineThroughOffset - (pathWidth * 0.5)),
                        path,
                        isHorizontal,
                        currentTextPathOffset,
                        currentTextPath->isClosedSubpath(0)));
                                             ...
                                             QPointF startPos)
{
    ...
    while (start < result.size()) {
        ...
        // Start pos and endpos are only used for inlinesize handling of hanging
        // punctuation.
        ...
        for (int j = start; j < i; j++) {
            ...
            if (cr.isHanging && !inlineSize.isAuto) {
                if (cr.lineStart == HangBehaviour && start == 0
                    && cr.anchored_chunk == true) {
    ...
    if (chunkShape->isTextNode()) {
        ...
        for (int i = currentIndex; i < j; i++) {
            if (result.at(i).addressable && result.at(i).hidden == false) {
                ...
                QPen pen(result.at(i).anchored_chunk?
                             result.at(i).isHanging? Qt::red: Qt::magenta:
                                                     result.at(i).lineEnd==NoChange?
                Qt::cyan: Qt::yellow); pen.setWidthF(0.1); painter.setPen(pen);
