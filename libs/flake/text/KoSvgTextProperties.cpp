...
QMap<QString, QString> KoSvgTextProperties::convertToSvgTextAttributes() const
{
    ...
    for (PropertyId id : alternates) {
        if (hasProperty(id)) {
            ...
            if (id == FontVariantHistoricalFormsId) {
                ...
                features.append(
                    QString("%1(%2)").arg(function).arg(property(id).toInt()));
            }
        }
    }
    if (!features.isEmpty()) {
    ...
    if (hasProperty(FontVariantPositionId)) {
        result.insert("font-variant-position",
                      featureMap.key(KoSvgText::FontVariantFeature(
                          property(FontVariantPositionId).toInt())));
    }
    if (hasProperty(FontVariantCapsId)) {
        result.insert("font-variant-caps",
                      featureMap.key(KoSvgText::FontVariantFeature(
                          property(FontVariantCapsId).toInt())));
    }
...
QStringList
KoSvgTextProperties::fontFileNameForText(QString text,
                                         QVector<int> &lengths) const
{
    if (text.isEmpty()) {
        return QStringList();
    }
    ...
    if (FcPatternGetString(fontSet->fonts[lastIndex], FC_FILE, 0, &fileValue)
        == FcResultMatch) {
