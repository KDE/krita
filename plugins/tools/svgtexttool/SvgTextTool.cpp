...
QVariant SvgTextTool::inputMethodQuery(Qt::InputMethodQuery query, QVariant argument) const
{
    if (canvas()) {
        return m_textCursor.inputMethodQuery(query, argument);
    } else {
        return KoToolBase::inputMethodQuery(query, argument);
    }
}
