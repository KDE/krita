...
#include "kis_polynomial_curve.h"
...
public:
    KisPolynomialSpline() {}
    KisPolynomialSpline(const QList<T_point> &a) {
        createSpline(a);
    }
...
struct KisPolynomialCurve::Data : public QSharedData {
    Data() {
        init();
    }
    ...
    mutable KisPolynomialSpline<QPointF, qreal> spline;
...
void KisPolynomialCurve::Data::updateSpline()
{
...
void KisPolynomialCurve::Data::invalidate()
{
    validSpline = false;
    validFTransfer = false;
    validU16Transfer = false;
...
void KisPolynomialCurve::Data::keepSorted()
{
    qSort(points.begin(), points.end(), pointLessThan);
}

qreal KisPolynomialCurve::Data::value(qreal x)
{
    updateSpline();
...
template<typename _T_, typename _T2_>
void KisPolynomialCurve::Data::updateTransfer(QVector<_T_>* transfer, bool& valid, _T2_ min, _T2_ max, int size)
{
...
struct KisPolynomialCurve::Private {
    QSharedDataPointer<Data> data;
};

KisPolynomialCurve::KisPolynomialCurve() : KisCurve(), d(new Private)
{
    d->data = new Data;
    QPointF p;
    p.rx() = 0.0; p.ry() = 0.0;
...
KisPolynomialCurve::KisPolynomialCurve(const QList<QPointF>& points) : KisCurve(), d(new Private)
{
    d->data = new Data;
...
KisPolynomialCurve::~KisPolynomialCurve()
{
    delete d;
}

bool KisPolynomialCurve::operator==(const KisPolynomialCurve& curve) const
{
    if (d->data == curve.d->data) return true;
    return d->data->points == curve.d->data->points;
}

qreal KisPolynomialCurve::value(qreal x) const
{
    return d->data->value(x);
}

QList<QPointF> KisPolynomialCurve::points() const
{
    return d->data->points;
}

void KisPolynomialCurve::setPoints(const QList<QPointF>& points)
{
    d->data.detach();
    d->data->points = points;
    d->data->invalidate();
...
void KisPolynomialCurve::setPoint(int idx, const QPointF& point)
{
    d->data.detach();
...
int KisPolynomialCurve::addPoint(const QPointF& point)
{
    d->data.detach();
...
void KisPolynomialCurve::removePoint(int idx)
{
    d->data.detach();
...
QString KisPolynomialCurve::toString() const
{
    QString sCurve;
...
//void KisPolynomialCurve::fromString(const QString& string)
//{
