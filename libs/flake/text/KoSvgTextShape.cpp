...
void KoSvgTextShape::relayout() const
{
    ...
    QVector<KoSvgTextChunkShapeLayoutInterface::SubChunk> textChunks =
        layoutInterface()->collectSubChunks(globalIndex, textDecorationInfo);
    QString text;
    ...
    if (raqm_set_text_utf16(layout, text.utf16(), text.size())) {
        ...
        for (KoSvgTextChunkShapeLayoutInterface::SubChunk chunk : textChunks) {
            ...
            if (!letterSpacing.isAuto) {
                raqm_set_letter_spacing_range(layout,
            ...
            if (!wordSpacing.isAuto) {
                raqm_set_word_spacing_range(layout,
        ...
        if (createTextDecor) {
            for (int i = 0; i < textDecorationInfo.size(); i++) {
                ...
                if (textDecorationInfo.at(i).startIndex <= cluster
                    && cluster < textDecorationInfo.at(i).startIndex
                            + textDecorationInfo.at(i).length) {
                    KoSvgText::TextDecorationInfo info =
                        textDecorationInfo.at(i);
                    ...
                    if (info.startIndex == cluster) {
                        ...
                        QRectF underline(
                            ...
                            QPointF(
                                width
                                    * (glyphs[g].ftface->size->metrics.x_scale
                                       / 65535.0),
                                width
                                    * (glyphs[g].ftface->size->metrics.y_scale
                                       / 65535.0)));
                        if (isHorizontal) {
                            info.lineWidth = ftTF.mapRect(underline).height();
                            info.underlineOffset = ftTF.map(QPointF(
                                0,
                                glyphs[g].ftface->underline_position
                                    * (glyphs[g].ftface->size->metrics.y_scale
                                       / 65535.0)));
                        } else {
                    ...
                    if (isHorizontal) {
                        ...
                        if (info.overlineOffset.y() > top) {
                            info.overlineOffset =
                                QPointF(info.overlineOffset.x(), top);
                        }
                        if (info.underlineOffset.y() < bottom
                            && info.positionHorizontal
                                == KoSvgText::UnderlineUnder) {
                            info.underlineOffset =
                                QPointF(info.underlineOffset.x(), bottom);
                        }
                    } else {
                        qreal left = charResult.path.boundingRect().left();
                        qreal right = charResult.path.boundingRect().right();
                        if (info.positionHorizontal
                            == KoSvgText::UnderlineRight) {
                            if (info.overlineOffset.x() > left) {
                                info.overlineOffset =
                                    QPointF(left, info.overlineOffset.y());
                            }
                            if (info.underlineOffset.x() < right) {
                                info.underlineOffset =
                                    QPointF(right, info.underlineOffset.y());
                            }
                        } else {
                            if (info.overlineOffset.x() < right) {
                                info.overlineOffset =
                                    QPointF(right, info.overlineOffset.y());
                            }
                            if (info.underlineOffset.x() > left) {
                                info.underlineOffset =
                                    QPointF(left, info.underlineOffset.y());
                            }
                        }
                    }
    ...
    for (KoSvgText::TextDecorationInfo info : textDecorationInfo) {
        ...
        for (quint32 cluster : info.clusters) {
            ...
            QTransform tf;
            tf.translate(charResult.finalPosition.x(),
                         charResult.finalPosition.y());
            ...
            if (info.line.testFlag(KoSvgText::DecorationLineThrough)) {
                linethrough.addPath(tf.map(p.translated(
                    (info.underlineOffset + info.overlineOffset) * 0.5)));
            }
        }
        // This is incorrect, because it now adds the textdecoration to the
        // shape that contains the first character, even when the textdecoration
        // might actually be from it's parent. I do not know how to solve this.
        globalIndex = 0;
        for (KoSvgTextChunkShapeLayoutInterface::SubChunk chunk : textChunks) {
            ...
            if (globalIndex == info.startIndex) {
                ...
                if (!underline.isEmpty()) {
                    ...
                    wrapper.shape()->layoutInterface()->addTextDecoration(
                ...
                if (!linethrough.isEmpty()) {
                    linethrough =
                        stroker.createStroke(linethrough).simplified();
                    wrapper.shape()->layoutInterface()->addTextDecoration(
    ...
    bool isHorizontal)
{
    ...
    if (baselineAdjust == KoSvgText::BaselineAuto) {
        baselineAdjust = KoSvgText::Baseline(
            properties.property(KoSvgTextProperties::DominantBaselineId)
                .toInt());
    }
    qreal fontSize =
    ...
    QVector<FT_Face> faces = KoFontRegistery::instance()->facesForCSSValues(
    ...
    if (hb_version_atleast(4, 0, 0)) {
        ...
        if (isHorizontal) {
            ...
            baselineTable.insert(
                ...
                (baseline - baselineTable.value(KoSvgText::BaselineAlphabetic))
                    * 0.5);
        ...
        hb_ot_metrics_get_position_with_fallback(
            ...
            &baseline2);
        superScript = QPointF(baseline * (-1.0 / 64) * (72. / res),
                              baseline2 * (-1.0 / 64) * (72. / res));
        hb_ot_metrics_get_position_with_fallback(
            font,
        ...
        hb_ot_metrics_get_position_with_fallback(
            ...
            &baseline2);
        subScript = QPointF(baseline * (-1.0 / 64) * (72. / res),
                            baseline2 * (-1.0 / 64) * (72. / res));
    } else {
        hb_ot_layout_get_baseline(font,
                                  HB_OT_LAYOUT_BASELINE_TAG_ROMAN,
                                  dir,
                                  script,
                                  HB_TAG_NONE,
    ...
    int offset = parentBaselineTable.value(baselineAdjust)
        - baselineTable.value(baselineAdjust);
    QPointF shift;
    if (isHorizontal) {
            ...
            if (stroke) {
                if (stroke->lineBrush().gradient()) {
                    ...
                    stroke->paint(
                        ...
                        painter);
                    if (!textDecorationsRest.isEmpty()) {
                        stroke->paint(KoPathShape::createShapeFromPainterPath(
                                          textDecorationsRest),
                                      painter);
                    }
                }
