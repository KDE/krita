...
{
    KisImageWSP image = m_view->image();    
    KisLayerSP layer = new KisPaintLayer(image.data(),  image->nextLayerName( i18n("Paint") ), OPACITY_OPAQUE_U8, image->colorSpace());
...
KisNodeSP KisLayerManager::addCloneLayer(KisNodeList nodes)
{
    ...
    Q_FOREACH (node, filteredNodes) {
        lastClonedNode = new KisCloneLayer(qobject_cast<KisLayer*>(node.data()), image.data(), image->nextLayerName( i18n("Clone") ), OPACITY_OPAQUE_U8);
...
KisNodeSP KisLayerManager::addShapeLayer(KisNodeSP activeNode)
{
    ...
    KisImageWSP image = m_view->image();
    KisShapeLayerSP layer = new KisShapeLayer(m_view->document()->shapeController(), image.data(), image->nextLayerName(i18n("Vector")), OPACITY_OPAQUE_U8);
...
KisNodeSP KisLayerManager::addAdjustmentLayer(KisNodeSP activeNode)
{
    ...
    KisDlgAdjustmentLayer dlg(adjl, adjl.data(), previewDevice, image->nextLayerName(i18n("Filter")), i18n("New Filter Layer"), m_view, qApp->activeWindow());
    dlg.resize(dlg.minimumSizeHint());
...
KisNodeSP KisLayerManager::addGeneratorLayer(KisNodeSP activeNode)
{
    ...
    KisDlgGeneratorLayer dlg(image->nextLayerName(i18n("Fill")), m_view, m_view->mainWindow(), 0, 0);
...
KisNodeSP KisLayerManager::addFileLayer(KisNodeSP activeNode)
{
    ...
    KisDlgFileLayer dlg(basePath, image->nextLayerName(i18n("File")), m_view->mainWindow());
    dlg.resize(dlg.minimumSizeHint());
