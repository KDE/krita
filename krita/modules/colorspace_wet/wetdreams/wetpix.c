  ...
  for (y = 0; y < height; y++)
      ...
      for (x = 0; x < width; x++)
	{
	  if (((x + y) & 3) == wet_phase) {
	    ...
	    if (highlight < 255)
	      {
		rgb_ptr[0] = 255 - (((255 - rgb_ptr[0]) * highlight) >> 8);
		rgb_ptr[1] = 255 - (((255 - rgb_ptr[1]) * highlight) >> 8);
		rgb_ptr[2] = 255 - (((255 - rgb_ptr[2]) * highlight) >> 8);
	      }
	  }
	  rgb_ptr += 3;
	  wet_ptr++;
		     ...
		     int width, int height)
{
  ...
  wet_composite (rgb, rgb_rowstride,
		 layer->buf + (y0 * layer->rowstride) + x0, layer->rowstride,
		 width, height, layer->mask);
}
...
WetLayer *
wet_layer_new (int width, int height)
{
  WetLayer *layer;
  ...
  layer->mask = g_new0 (byte, ((width + 3) >> 2) * ((height + 3) >> 2));

  return layer;
}

void
wet_layer_clear (WetLayer *layer)
{
  int x, y;
...
WetPack *
wet_pack_new (int width, int height)
{
  WetPack *pack;
...
void
wet_pix_to_double (WetPixDbl *dst, WetPix *src)
{
...
void
wet_pix_from_double (WetPix *dst, WetPixDbl *src)
{
  int v;
