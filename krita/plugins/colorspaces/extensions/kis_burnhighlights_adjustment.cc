...
public:
 	...
 	void transform(const quint8 *srcU8, quint8 *dstU8, qint32 nPixels) const
 	{
        ...
        while(nPixels > 0)
        {   
            r = exposure * KoColorSpaceMaths<_channel_type_, float>::scaleToA(src->red);
            g = exposure * KoColorSpaceMaths<_channel_type_, float>::scaleToA(src->green);
            b = exposure * KoColorSpaceMaths<_channel_type_, float>::scaleToA(src->blue);
