...
KisEmbossFilterConfigurationWidget::KisEmbossFilterConfigurationWidget( KisFilter* nfilter, QWidget * parent, const char * name) : KisFilterConfigurationWidget( nfilter, parent, name )
{       
        ...
        kefcw = new KisEmbossFilterConfigurationBaseWidget((QWidget*)this);
	
        //This adds the configuration widget to the dialog
        
        widgetLayout -> addWidget(kefcw, 0 , 0);

        //refresh the filter preview if the parameters for the filter are changed
        
        connect((QObject*)kefcw->depthSpinBox, SIGNAL(valueChanged(int)), filter(), SLOT(refreshPreview()));
}
