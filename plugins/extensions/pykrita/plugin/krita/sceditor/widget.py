...
try:
    from PyQt6.QtCore import QCoreApplication, QLine, Qt
    from PyQt6.QtGui import (QBrush, QColor, QFont, QKeyEvent, QTextBlockUserData,
...
class EditorView(QPlainTextEdit):
    ...
    def __init__(self, parent=None, text=None,
        ...
        self.draw_line = True
        self.print_width = self.fontMetrics().horizontalAdvance("x" * 78)
    ...
    def keyPressEvent(self, event):
        ...
        if self.disable_shortcuts and \
                m & Qt.KeyboardModifier.ControlModifier and k in [Qt.Key.Key_A, Qt.Key.Key_R,
                                                 Qt.Key.Key_C, Qt.Key.Key_K,
                                                 Qt.Key.Key_X, Qt.Key.Key_V,
            ...
            if self.backspace_pressed():
                return
        elif k == Qt.Key.Key_Period or \
                (k == Qt.Key.Key_Space and event.modifiers() == Qt.KeyboardModifier.ControlModifier):
            QPlainTextEdit.keyPressEvent(self, event)
            self.autocomplete_pressed()
            return
        elif k in [Qt.Key.Key_ParenLeft, Qt.Key.Key_BraceLeft, Qt.Key.Key_BracketLeft]:
            QPlainTextEdit.keyPressEvent(self, event)
            self.paren_opened(k)
            return
        QPlainTextEdit.keyPressEvent(self, event)
