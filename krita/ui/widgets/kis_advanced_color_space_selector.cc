...
void KisAdvancedColorSpaceSelector::fillDescription()
{
    ...
    QString estimatedsRGB = i18nc("This is for special Gamma types that LCMS can't differentiate between","Estimated Gamma: sRGB, L* or rec709 trc");
    ...
    if (profileList.isEmpty()) {
        ...
        d->colorSpaceSelector->lbltrc->setToolTip(i18nc("@info:tooltip","Estimated Gamma can't be retrieved for CMYK"));
        ...
        d->colorSpaceSelector->lbltrc->setToolTip(i18nc("@info:tooltip","Estimated Gamma can't be retrieved for YCRCB"));
        d->colorSpaceSelector->lbltrc->setText(estimatedGamma+notApplicable);
    }
    ...
    if (currentModelStr == "RGBA") {
        d->colorSpaceSelector->textProfileDescription->appendPlainText(i18nc("If the selected model is RGB","RGB, standing for Red, Green, Blue, is the colour model used by screens and other light-based media. It is an additive colour model, meaning that adding colours together makes them brighter. This colour model is the most extensive of all colour models, and is recommended as a model for working files, that you can later convert to other spaces. RGB is also the recommended space for HDR editing."));
            ...
            d->colorSpaceSelector->textProfileDescription->appendPlainText(i18nc("If the selected model is Grayscale","Grayscale, as it states, allows only for gray values and transparent values. The benefit of this is that it will take up twice as little RAM and harddrive space as any given RGB image of the same bit-depth. Useful for inking and greyscale images."));
    } else if (currentModelStr == "LABA") {
    ...
    if (currentDepthStr == "U8") {
            d->colorSpaceSelector->textProfileDescription->appendPlainText(i18nc("When the selected Bitdepth is 8","8bit: The default amount of colours per channel. Each channel will have 256 values available, leading to a total amount of 256*amount of channels. Recommended to use for images intended for the web, or otherwise simple images."));
