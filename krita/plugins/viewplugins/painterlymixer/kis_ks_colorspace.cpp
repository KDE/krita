...
const double g_D50[3][WLS_NUMBER] = {
	{
	...
	: KoIncompleteColorSpace<KSTraits, KoRGB16Fallback>("kscolorspace", "", KoColorSpaceRegistry::instance())
{
	...
	*/
	m_D65_D50 = new double*[3];
	for (int i = 0; i < 3; i++) {
		m_D65_D50[i] = new double[3];
		for (int j = 0; j < 3; j++)
			m_D65_D50[i][j] = g_D65_D50[i][j];
	}

	m_D50_D65 = new double*[3];
	for (int i = 0; i < 3; i++) {
		m_D50_D65[i] = new double[3];
		for (int j = 0; j < 3; j++)
			m_D50_D65[i][j] = g_D50_D65[i][j];
	}
}
...
void KisKSColorSpace::fromRgbA16(const quint8 * srcU8, quint8 * dstU8, quint32 nPixels) const
{
	...
	double XYZ50[3], XYZ65[3], REF[ncols];

	for (quint32 i = 0; i < nPixels; i++) {
		cmsDoTransform(BGR_XYZ, const_cast<quint16*>(src16), XYZ50, 1);

		maths::mult(3, 3, m_D50_D65, XYZ50, XYZ65);
		maths::simplex(3, ncols, m_profile->matrix(), REF, XYZ65);
// 		maths::simplex(3, ncols, m_profile->matrix(), REF, XYZ50);
...
void KisKSColorSpace::toRgbA16(const quint8 * srcU8, quint8 * dstU8, quint32 nPixels) const
{
	...
	double XYZ65[3], XYZ50[3], REF[ncols];

	for (quint32 i = 0; i < nPixels; i++) {
		maths::computeReflectance(ncols, srcf, REF);

// 		maths::mult(3, ncols, m_profile->matrix(), REF, XYZ50);
		maths::mult(3, ncols, m_profile->matrix(), REF, XYZ65);
		maths::mult(3, 3, m_D65_D50, XYZ65, XYZ50);
