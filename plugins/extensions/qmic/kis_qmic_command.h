...
class KisQmicCommand : public QObject, public KUndo2Command
{
    Q_OBJECT
public:
    KisQmicCommand(const QString &gmicCommandString, QSharedPointer< QVector<gmic_image<float> > > images, KisQmicDataSP data, const QByteArray &customCommands = QByteArray());
    virtual ~KisQmicCommand();
...
private:
    ...
    QSharedPointer<QVector<gmic_image<float> > > m_images;
    KisQmicDataSP m_data;
