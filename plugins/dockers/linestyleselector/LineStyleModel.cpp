...
LineStyleModel::LineStyleModel( QObject * parent )
...
int LineStyleModel::rowCount( const QModelIndex &/*parent*/ ) const
{
    return m_styles.count() + ( m_hasTempStyle ? 1 : 0 );
}

QVariant LineStyleModel::data( const QModelIndex &index, int role ) const
{
    if( ! index.isValid() )
         return QVariant();
...
bool LineStyleModel::addCustomStyle( const QVector<qreal> &style )
{
...
int LineStyleModel::setLineStyle( Qt::PenStyle style, const QVector<qreal> &dashes )
{
