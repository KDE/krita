...
struct KisBasicInternalColorSelector::Private
{
    ...
    KisPaletteChooser *colorSetChooser = 0;
};

KisBasicInternalColorSelector::KisBasicInternalColorSelector(QWidget *parent, KoColor color, Config config, const QString &caption, const KoColorDisplayRendererInterface *displayRenderer)
    : QDialog(parent)
    , m_d(new Private)
{
    setModal(config.modal);
    this->setFocusPolicy(Qt::ClickFocus);
    m_ui = new Ui_WdgBasicInternalColorSelector();
    m_ui->setupUi(this);
    ...
    if (config.paletteBox) {
        ...
        m_d->colorSetChooser = new KisPaletteChooser(this);
...
KisBasicInternalColorSelector::~KisBasicInternalColorSelector()
{
    delete m_ui;
}

void KisBasicInternalColorSelector::slotColorUpdated(KoColor newColor)
{
...
void KisBasicInternalColorSelector::slotSetColorFromPatch(KoColorPatch *patch)
{
    slotColorUpdated(patch->color());
}

void KisBasicInternalColorSelector::colorSpaceChanged(const KoColorSpace *cs)
{
...
void KisBasicInternalColorSelector::lockUsedColorSpace(const KoColorSpace *cs)
{
    colorSpaceChanged(cs);
    m_d->lockUsedCS = true;
}

void KisBasicInternalColorSelector::setDisplayRenderer(const KoColorDisplayRendererInterface *displayRenderer)
{
    if (displayRenderer) {
        m_d->displayRenderer = displayRenderer;
        m_ui->visualSelector->setDisplayRenderer(displayRenderer);
...
KoColor KisBasicInternalColorSelector::getModalColorDialog(const KoColor color, QWidget* parent, QString caption)
{
    Config config = Config();
    KisBasicInternalColorSelector dialog(parent, color, config, caption);
    dialog.setPreviousColor(color);
    dialog.exec();
    return dialog.getCurrentColor();
}

KoColor KisBasicInternalColorSelector::getCurrentColor()
{
    return m_d->currentColor;
}

void KisBasicInternalColorSelector::chooseAlpha(bool chooseAlpha)
{
    m_d->chooseAlpha = chooseAlpha;
}

void KisBasicInternalColorSelector::slotConfigurationChanged()
{
    //m_d->canvas->displayColorConverter()->
    //slotColorSpaceChanged(m_d->canvas->image()->colorSpace());
}

void KisBasicInternalColorSelector::slotLockSelector()
{
    m_d->allowUpdates = false;
}

void KisBasicInternalColorSelector::setPreviousColor(KoColor c)
{
    m_d->previousColor = c;
}

void KisBasicInternalColorSelector::reject()
{
    slotColorUpdated(m_d->previousColor);
    QDialog::reject();
}

void KisBasicInternalColorSelector::updateAllElements(QObject *source)
{
    //update everything!!!
...
void KisBasicInternalColorSelector::endUpdateWithNewColor()
{
    m_d->allowUpdates = true;
}

void KisBasicInternalColorSelector::focusInEvent(QFocusEvent *)
{
    //setPreviousColor();
}

void KisBasicInternalColorSelector::slotFinishUp()
{
...
void KisBasicInternalColorSelector::slotSetColorFromHex()
{
    slotColorUpdated(m_d->sRGB);
}

void KisBasicInternalColorSelector::slotChangePalette(KoColorSet *set)
{
    if (!set) {
        return;
    }
...
void KisBasicInternalColorSelector::slotSetColorFromColorList()
{
...
void KisBasicInternalColorSelector::slotSetColorFromColorSetEntry(KoColorSetEntry entry)
{
    slotColorUpdated(entry.color());
}

void KisBasicInternalColorSelector::showEvent(QShowEvent *event)
{
    updateAllElements(0);
