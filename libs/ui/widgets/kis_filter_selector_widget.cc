...
void KisFilterSelectorWidget::setFilter(KisFilterSP f)
{
    ...
    int lastBookmarkedFilterConfiguration = KisConfig().readEntry<int>("lastBookmarkedFilterConfiguration/" + f->id(), 0);
...
void KisFilterSelectorWidget::slotBookmarkedFilterConfigurationSelected(int index)
{
    if (d->currentFilterConfigurationWidget) {
        ...
        d->currentFilterConfigurationWidget->setConfiguration(config);
        if (d->currentFilter && index != KisConfig().readEntry<int>("lastBookmarkedFilterConfiguration/" + d->currentFilter->id(), 0)) {
            KisConfig().writeEntry<int>("lastBookmarkedFilterConfiguration/" + d->currentFilter->id(), index);
        }
    }
}
