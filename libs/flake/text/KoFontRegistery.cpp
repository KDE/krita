                                                    ...
                                                    QString language)
{
    ...
    for (QString family : families) {
        const FcChar8 *vals =
            reinterpret_cast<FcChar8 *>(family.toUtf8().data());
    ...
    FcChar8 *fileValue = 0;
    FcFontSet *fontSet =
        FcFontSort(FcConfigGetCurrent(), p, FcTrue, &charSet, &result);
    ...
    if (text.isEmpty()) {
        for (QString family : families) {
            for (int j = 0; j < fontSet->nfont; j++) {
                ...
                if (FcPatternGetString(fontSet->fonts[j],
                                       FC_FAMILY,
                                       0,
                                       &fileValue)
                    == FcResultMatch) {
    ...
    for (int i = 0; i < lengths.size(); i++) {
        ...
        if (FT_New_Face(d->library,
                        fontFileNames.at(i).toUtf8().data(),
                        0,
                        &face)
            == 0) {
            faces.append(face);
    ...
    for (FT_Face face : faces) {
        if (!FT_IS_SCALABLE(face)) {
            ...
            for (int i = 0; i < face->num_fixed_sizes; i++) {
                int newDelta =
                    qAbs((fontSizePixels)-face->available_sizes[i].height);
                ...
                if (FT_HAS_COLOR(face)) {
                    qreal scale =
                        qreal(fontSizePixels
                              / face->available_sizes[selectedIndex].x_ppem);
                    FT_Matrix matrix;
                    matrix.xx = scale;
                    matrix.yy = scale;
                    FT_Vector v;
            ...
            errorCode =
                FT_Set_Char_Size(face, size * ftFontUnit, 0, xRes, yRes);
        }
