...
KisQmicCommand::KisQmicCommand(const QString &gmicCommandString, QSharedPointer< QVector<gmic_image<float> > > images, KisQmicDataSP data, const QByteArray &customCommands):
...
KisQmicCommand::~KisQmicCommand()
{
    dbgPlugins << "Destructor: " << this;
}


void KisQmicCommand::undo()
{
    // do nothing
}

void KisQmicCommand::redo()
{
    if (m_firstRedo)
    {
        m_firstRedo = false;
        ...
        {
            dbgPlugins << "G'MIC Input image " << i << " = " << KisQmicCommand::gmicDimensionString(m_images->at[i]) << ", buffer : " << m_images->at[i]._data;
        }
        ...
        for (unsigned int i = 0; i < m_images->_width; ++i) {
            dbgPlugins << "   Output image "<< i << " = " << KisQmicCommand::gmicDimensionString(m_images->at[i]_data) << ", buffer : " << m_images->at[i]_data._data;
        }
...
QString KisQmicCommand::gmicDimensionString(const gmic_image<float>& img)
{
    return QString("%1x%2x%3x%4").arg(img._width).arg(img._height).arg(img._depth).arg(img._spectrum);
}

bool KisQmicCommand::isSuccessfullyDone()
{
