...
KisCurveOpOptionModel::KisCurveOpOptionModel(lager::cursor<KisCurveOpOptionData> _optionData)
    ...
    , LAGER_QT(curveStrokeHistorySize) {_optionData[&KisCurveOpOptionData::curve_stroke_history_size].zoom(kiszug::lenses::do_static_cast<int, qreal>)}
    , LAGER_QT(curveLineWidth) {_optionData[&KisCurveOpOptionData::curve_line_width].zoom(kiszug::lenses::do_static_cast<int, qreal>)}
