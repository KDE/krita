...
void DocumentStructureTest::predefinedMetaData_test() 
{
    ...
    QString error;
    QVERIFY(odfReadStore.loadAndParse("meta.xml", metaDoc, error));
    QVERIFY(documentInfo->loadOasis(metaDoc));
    ...
    QCOMPARE(keywords[0].trimmed(), QString("First keyword"));
    QCOMPARE(keywords[1].trimmed(), QString("Second keyword"));
    QCOMPARE(keywords[2].trimmed(), QString("Third keyword"));
