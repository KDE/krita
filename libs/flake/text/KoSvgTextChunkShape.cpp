...
struct KoSvgTextChunkShape::Private::LayoutInterface : public KoSvgTextChunkShapeLayoutInterface
{
    ...
    QVector<SubChunk> collectSubChunks(int &globalIndex, QVector<KoSvgText::TextDecorationInfo> &textDecorationInfo, bool textInPath) const override {
        ...
        if (q->s->properties.hasProperty(KoSvgTextProperties::TextDecorationLineId)) {
            textDecorInfo.line = q->s->properties.propertyOrDefault(KoSvgTextProperties::TextDecorationLineId).value<KoSvgText::TextDecorations>();
        }
        if (textDecorInfo.line != KoSvgText::DecorationNone) {
            using namespace KoSvgText;
            textDecorInfo.line = q->s->properties.property(KoSvgTextProperties::TextDecorationLineId).value<TextDecorations>();
            textDecorInfo.style = TextDecorationStyle(
                        q->s->properties.propertyOrDefault(KoSvgTextProperties::TextDecorationStyleId).toInt());
        ...
        if (isTextNode()) {
            ...
            Q_FOREACH (KoShape *shape, q->shapes()) {
                ...
                result += chunkShape->layoutInterface()->collectSubChunks(globalIndex, textDecorationInfo, textInPath);
            }
        }
