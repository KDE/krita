...
void KoTextEditor::Private::documentCommandAdded()
{
    ...
    if (commandStack.isEmpty()) {
        ...
        kDebug(32500) << "empty stack, will push a new headCommand on both commandStack and application's stack. title: " << commandTitle;
        ...
        kDebug(32500) << "we have a headCommand on the commandStack but need a new child command. we will push it only on the commandStack: " << commandTitle;
        ...
        kDebug(32500) << "we are in subsequent keyPress/delete state and still received a signal. we need to create a new headCommand: " << commandTitle;
        kDebug(32500) << "so we pop the current one and push the new one on both the commandStack and the application's stack";
        commandStack.pop();
...
void KoTextEditor::Private::updateState(KoTextEditor::Private::State newState, QString title)
{
    ...
    if (newState == NoOp && !commandStack.isEmpty()) {
        ...
        kDebug(32500) << "we are in a macro and update the state to NoOp. this means that the command on top of the commandStack is finished. we should pop it";
        kDebug(32500) << "commandStack count before: " << commandStack.count();
        commandStack.pop();
        kDebug(32500) << "commandStack count after: " << commandStack.count();
        if (commandStack.isEmpty()) {
            kDebug(32500) << "we have no more commands on the commandStack. the macro is complete. next signal induced command will need to be parented to a new headCommand. Also the editor should go to NoOp";
            addNewCommand = true;
            editorState = NoOp;
        }
    ...
    if (editorState != newState || commandTitle != title) {
        ...
        kDebug(32500) << "we are not in a custom state but change the command";
        kDebug(32500) << "commandStack count: " << commandStack.count();
        if (!commandStack.isEmpty()) {
            kDebug(32500) << "the commandStack is not empty. however the command on it is not a macro. so we pop it and ask to recreate a new one: " << title;
            commandStack.pop();
            addNewCommand = true;
        }
    }
...
void KoTextEditor::endEditBlock()
{
    ...
    if (!d->customCommandCount) {
        ...
        d->updateState(KoTextEditor::Private::NoOp);
        kDebug(32500) << "done setting the state. editorState: " << d->editorState << " commandStack count: " << d->commandStack.count();
    }
}
