...
public:
    ...
    virtual KisPixelRO toKisPixelRO(const Q_UINT8 *src)
        { return KisPixelRO (src, src + PIXEL_ALPHA * sizeof(half), this); }
    virtual KisPixel toKisPixel(Q_UINT8 *src)
        { return KisPixel (src, src + PIXEL_ALPHA * sizeof(half), this); }
...
public:
     ...
     */
    virtual KisID id() const { return KisID("RGBAF16HALF", i18n("RGB/Alpha (16-bit float/channel)")); };
